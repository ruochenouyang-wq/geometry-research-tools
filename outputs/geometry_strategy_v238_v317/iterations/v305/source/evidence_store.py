...
class ResearchGraph:
    ...
    def add_relation(self, kind, sources, target, *, conditions=None, scope=None, rule=None):
        ...
        if type(sources) is not list or not 1 <= len(sources) <= 32 or len(set(sources)) != len(sources):
            raise ValueError('Require one to 32 distinct source nodes')
        if any(type(ref) is not str or ref not in self.nodes for ref in sources) or target not in self.nodes:
    ...
    def relation_status(self, ref):
        ...
        return {'reference':ref,'kind':relation['kind'],'verified':False,
                'status':'heuristic' if relation['kind']=='heuristic' else 'unverified_no_rule'}
