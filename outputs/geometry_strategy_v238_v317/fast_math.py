...
def trial_statistics(powers,coefficients):
    ...
    mass = energy = operator_norm = F(0)
    # One coefficient product supplies all three symmetric quadratic forms.
    for i,vi in enumerate(v):
        ...
        for j in range(i,len(v)):
            ...
            weight = vi*v[j]
            if i != j: weight *= 2
            _count('statistics_coefficient_products')
            _count('statistics_gram_contractions',3)
            mass += weight*M[i][j]
            energy += weight*H[i][j]
            operator_norm += weight*R[i][j]
...
@_cached(64)
def _ldl_factors(ss):
    """Extend immutable positive LDL factors by one exact Gram border."""
    n = len(ss)
    H = _trial_matrices(ss)[1]
    old_L,old_D = _ldl_factors(ss[:-1]) if n > 1 else ((),())
    L = [list(row)+[F(0)] for row in old_L]+[[F(0)]*(n-1)+[F(1)]]
    D = list(old_D)
    for k in range(n-1):
        _count('ldl_new_offdiagonal_entries')
        L[-1][k] = (H[-1][k]-sum((L[-1][j]*L[k][j]*D[j]
                                 for j in range(k)),F(0)))/D[k]
    d = H[-1][-1]-sum((L[-1][k]**2*D[k] for k in range(n-1)),F(0))
    if d <= 0:
        raise ValueError('Candidate inverse-iteration matrix must be positive definite')
    D.append(d)
    _count('ldl_new_pivots')
    return tuple(map(tuple,L)),tuple(D)


def _solve_factors(factors,rhs):
    L,D = factors
    n = len(D)
    if len(rhs) != n: raise ValueError('Right-hand side length mismatch')
    y = []
    for i in range(n):
...
def propose_trial(powers,precision_bits=112,iterations=14):
    """Same rounded exact proposal as the baseline; factors are reusable."""
    ss = enriched.exponents(powers)
    direct.integer(precision_bits,32,256,'precision_bits')
    direct.integer(iterations,0,64,'iterations')
    started = perf_counter()
    M,_,_ = _trial_matrices(ss)
    factors = _ldl_factors(ss)
    ...
    for _ in range(iterations):
        ...
        x = _solve_factors(factors,rhs)
        scale = max(abs(z) for z in x)
        if not scale: raise ArithmeticError('Zero inverse iteration proposal')
