...
FIRST_RELEASE = ROOT.parent / 'geometry_strategy_v317_release'
DATA = ROOT / 'evaluation'
VERSION = 'geometry_runtime_preregistered_v3'
REPETITIONS = 5
METHODS = ['old_default', 'fixed_increasing', 'validation_selected_fixed', 'frozen_v317', 'first_release', 'release']
LONG_TERM_BASELINES = METHODS[:4]
CONFIGS = [
...
def _forbid_old_seed(path):
    path = Path(path).resolve()
    if path.name == 'private_seed.json' and any(path.is_relative_to(root.resolve()) for root in (OLD,FIRST_RELEASE)):
        raise RuntimeError('Neither previous private seed may ever be read')
...
def first_release_regression_cases():
    """The previous 14 disclosed cases are public regression, not new holdout."""
    result = []
    for source in read_json(FIRST_RELEASE/'evaluation/holdout_instances.json'):
        item = deepcopy(source)
        item.update(id='FIRST_'+source['id'],classification='first_release_disclosed_holdout_now_regression')
        result.append(item)
    return result


def known_public_tasks():
    result = []
    for source, cases in (('public_validation',validation_cases()),('old_six_regression',regression_cases()),
                          ('first_release_fourteen_regression',first_release_regression_cases())):
        result.extend({'source':source,'case_id':c['id'],'task':deepcopy(c['task'])} for c in cases)
    return result


def classify_draws(items):
    """Do not reject/resample a repeated task. Label it transparently instead."""
    known = {}
    for entry in known_public_tasks():
        known.setdefault(canonical(entry['task']),[]).append({'source':entry['source'],'case_id':entry['case_id']})
    seen = {}
    for item in items:
        key = canonical(item['task'])
        overlaps = deepcopy(known.get(key,[]))
        duplicate = seen.get(key)
        item.update(prior_public_overlap=overlaps,duplicate_of_current_draw=duplicate,
                    independent_new_instance=not overlaps and duplicate is None)
        item['classification'] = ('public_task_overlap_regression' if overlaps else
                                   'duplicate_current_draw' if duplicate else 'new_within_family_parameters_only')
...
def public_rules():
    return {
        ...
        'holdout': {
            ...
            'prior_overlap': 'Keep the identical sampler without rejection/resampling. Label any exact task repeated from prior public validation, old six or previous fourteen as regression; label duplicates within this draw. Only unique nonoverlapping cases count as independent new instances. All fourteen remain in costs and full success denominators.',
        },
        ...
        'session_design': 'new subprocess per method/repetition; six methods rotated by repetition, five repeats; same task order rotated by repetition; session-local caches allowed, no cache across sessions; all imports/constructor setup and first-use lazy imports charged; exactly one release module root per process, independently checked',
        ...
        'reward_rules': {
            ...
            'resources': 'The long-term 80-iteration runtime gate requires equal robust successes, no per-case success loss and >=20% median full-session wall and CPU reduction against each applicable original baseline: old_default, fixed_increasing, validation_selected_fixed, frozen_v317. first_release is an incremental safety reference and is not a new >=20% hurdle. Validation preparation is charged to its selected baseline once; online-only results remain separate.',
            'incremental_first_release': 'Against first_release require no robust-success losses, no reliability regression, and both median AND five-session total full wall/CPU ratios <=1.0; no 20% compounding hurdle.',
            'old_six_cold_regression': 'On the original six public regression tasks require no robust-success loss against frozen_v317 and both median AND five-session total full wall/CPU ratios <=1.0, with all five raw values retained. No rounding or timing tolerance may turn a failure into pass.',
            'independent_subset': 'Report truly new unique instances separately, including successes and summed online wall/CPU per repeat. Whole-session initialization and output costs cannot be attributed to individual cases and remain whole-session only.',
...
def initialize():
    ...
    write_json(DATA / 'known_public_tasks.json',known_public_tasks())
...
def load_adapter(method, evidence_root, configuration=None):
    ...
    if method in ('old_default', 'fixed_increasing', 'validation_selected_fixed', 'validation_candidate'):
        ...
        elif method in ('release','first_release'):
            target = ROOT if method=='release' else FIRST_RELEASE
            other = FIRST_RELEASE if method=='release' else ROOT
            # The script directory is otherwise on sys.path even in a baseline
            # process. Remove both candidate roots, then expose exactly one.
            if any(Path(getattr(sys.modules[name],'__file__','/')).resolve().is_relative_to(other.resolve())
                   for name in ('release_service','release_backend','request_runtime') if name in sys.modules):
                raise RuntimeError('Cross-release module contamination; a clean subprocess is required')
            sys.path[:] = [p for p in sys.path if Path(p or os.getcwd()).resolve() not in (ROOT.resolve(),FIRST_RELEASE.resolve())]
            sys.path.insert(0,str(target))
            import release_service
            if Path(release_service.__file__).resolve() != (target/'release_service.py').resolve():
                raise RuntimeError('Wrong release implementation imported')
            ResearchService = release_service.ResearchService
        else:
            raise ValueError('Unknown method: ' + method)
        service = ResearchService(evidence_root=evidence_root, cache=False)
        def adapter(task, index=None):
            return service.evaluate_task(deepcopy(task))
    adapter.implementation_root = str(ROOT if method=='release' else FIRST_RELEASE if method=='first_release' else OLD)
...
def worker(spec):
    ...
    with tempfile.TemporaryDirectory(prefix='geometry-evaluation-') as evidence:
        ...
        rows, probes, implementation_root = [], [], None
        if setup['execution_ok']:
            adapter, verifier, normalize = setup.pop('response')
            implementation_root = adapter.implementation_root
        ...
        report = {'method': spec['method'], 'repetition': spec['repetition'],
                  ...
                  'release_modules_loaded':{name:str(Path(getattr(module,'__file__')).resolve()) for name,module in list(sys.modules.items())
                                            if name in ('release_service','release_backend','request_runtime') and getattr(module,'__file__',None)},
...
def summarize(sessions, cases, preparation=None, phase=None):
    summaries = {}
    for method in dict.fromkeys(s['method'] for s in sessions):
        ...
        for item in cases:
            ...
            per_case.append({'case_id': item['id'], 'family': item['family'], 'classification': item['classification'],
                ...
                'independent_new_instance':item.get('independent_new_instance',False),
                'prior_public_overlap':item.get('prior_public_overlap',[]),
                ...
                'cpu_seconds': distribution([r['accounted_cpu_seconds'] for r in subset]),
        ...
        independent_ids = {c['id'] for c in cases if c.get('independent_new_instance') is True}
        independent_cases = [c for c in per_case if c['case_id'] in independent_ids]
        independent_subset = {'case_ids':sorted(independent_ids),'cases':len(independent_ids),
            'robust_successes':sum(c['robust_success'] for c in independent_cases),
            'successful_runs':sum(c['successes'] for c in independent_cases),
            'per_case':independent_cases,
            'online_wall_seconds_per_repeat':distribution([sum(r['accounted_wall_seconds'] for r in s['rows'] if r['case_id'] in independent_ids) for s in group]),
            'online_cpu_seconds_per_repeat':distribution([sum(r['accounted_cpu_seconds'] for r in s['rows'] if r['case_id'] in independent_ids) for s in group]),
            'full_session_cost_attribution':'not allocated to cases; initialization/output/probes remain whole-session costs'}
        independent_subset['online_wall_seconds_total'] = sum(independent_subset['online_wall_seconds_per_repeat']['values'])
        independent_subset['online_cpu_seconds_total'] = sum(independent_subset['online_cpu_seconds_per_repeat']['values'])
        summaries[method] = {
            'independent_new_subset':independent_subset,
            ...
            'online_cpu_seconds': distribution([sum(r['accounted_cpu_seconds'] for r in s['rows']) for s in group]),
            'setup_wall_seconds': distribution([s['setup'].get('wall_seconds',0) for s in group]),
            'setup_cpu_seconds': distribution([s['setup'].get('cpu_seconds',0) for s in group]),
    ...
    if 'release' in summaries:
        ...
        for method in METHODS[:-1]:
            ...
            reliability = not new['false_accepts'] and not new['probe_execution_failures'] and not new['worker_failures']
            no_loss = not comparison['robust_success_losses']
            ratios = [comparison['median_full_session_wall_ratio_release_over_baseline'],
                      comparison['median_full_session_cpu_ratio_release_over_baseline'],
                      comparison['total_experiment_wall_ratio_including_preparation'],
                      comparison['total_experiment_cpu_ratio_including_preparation']]
            comparison['incremental_nonregression_all_four_time_ratios'] = bool(no_loss and reliability and all(r<=1.0 for r in ratios))
            applicable = method in LONG_TERM_BASELINES and new['robust_successes']==old['robust_successes']
            comparison['long_term_20_percent_gate_applicable'] = applicable
            comparison['long_term_20_percent_gate_passed'] = bool(no_loss and reliability and ratios[0]<=0.8 and ratios[1]<=0.8) if applicable else None
    relevant = [c for c in comparisons.values() if c['long_term_20_percent_gate_applicable']]
    gates = {'phase':phase,
             'long_term_original_baseline_runtime_gate':bool(relevant and all(c['long_term_20_percent_gate_passed'] for c in relevant)),
             'long_term_gate_excludes_first_release_by_preregistration':True,
             'first_release_incremental_no_regression':comparisons.get('first_release',{}).get('incremental_nonregression_all_four_time_ratios'),
             'old_six_cold_no_regression_vs_v317':comparisons.get('frozen_v317',{}).get('incremental_nonregression_all_four_time_ratios') if phase=='regression' else None,
             'old_six_gate_scope':'applies only to the original six regression phase; full wall/CPU medians AND five-session sums <=1.0, no lost robust successes',
             'statistical_significance_claimed':False}
    return {'methods': summaries, 'release_against_every_baseline': comparisons,'preregistered_gates':gates}
...
def verify_old_preservation():
    ...
    for name, source in (('PRESERVED_V317.json',OLD),('PRESERVED_FIRST_RELEASE.json',FIRST_RELEASE)):
        ...
        for relative, expected in manifest['files'].items():
            ...
            if path.name == 'private_seed.json':
                private_references.append({'root':str(source),'path': relative, 'preexisting_hash': expected, 'not_read': True})
                continue
            if file_digest(path) != expected:
                raise RuntimeError('Frozen old file changed: '+str(path))
            checked += 1
    return {'checked': checked, 'old_private_seed_references_without_reading': private_references}


def checked_acceptance_regression():
    """Bind the final pre-holdout cold regression selected by the root task.

    This checks report identity and its recorded gate, without changing numeric
    criteria. The release audit separately recomputes the gate from raw data.
    """
    pointer = read_json(DATA/'acceptance_regression.json')
    if type(pointer) is not dict or set(pointer) != {'report_path','sha256'}:
        raise RuntimeError('Acceptance regression pointer requires exactly report_path and sha256')
    relative, expected = pointer['report_path'], pointer['sha256']
    if type(relative) is not str or type(expected) is not str or len(expected)!=64 or any(c not in '0123456789abcdef' for c in expected):
        raise RuntimeError('Invalid acceptance regression path or SHA-256')
    parts = Path(relative).parts
    if (Path(relative).is_absolute() or len(parts)!=3 or parts[0]!='evaluation' or
            not parts[1].startswith('regression_') or parts[2]!='report.json' or '..' in parts):
        raise RuntimeError('Acceptance regression report must be ROOT-relative evaluation/regression_<timestamp>/report.json')
    path = (ROOT/relative).resolve()
    if not path.is_relative_to(ROOT.resolve()) or path.relative_to(ROOT.resolve()).as_posix()!=relative:
        raise RuntimeError('Acceptance regression report path must not redirect outside its named location')
    if file_digest(path)!=expected:
        raise RuntimeError('Acceptance regression report SHA-256 mismatch')
    report = read_json(path)
    gates = report.get('results',{}).get('preregistered_gates',{})
    if (report.get('phase')!='regression' or report.get('version')!=VERSION or
            report.get('protocol_digest')!=digest(public_rules()) or gates.get('phase')!='regression' or
            gates.get('old_six_cold_no_regression_vs_v317') is not True):
        raise RuntimeError('Acceptance report must be this protocol\'s regression phase with cold gate true')
    return pointer
...
def write_freeze_manifest():
    ...
    acceptance_regression = checked_acceptance_regression()
    preserved = verify_old_preservation()
    files = sorted(p for p in ROOT.rglob('*.py') if '__pycache__' not in p.parts)
    files += [ROOT/'PRESERVED_V317.json',ROOT/'PRESERVED_FIRST_RELEASE.json', ROOT/'EVALUATION.md', DATA/'protocol.json', DATA/'validation_cases.json',DATA/'known_public_tasks.json',
              DATA/'validation_selection.json', DATA/'seed_commitment.json', DATA/'acceptance_regression.json',
              ROOT/acceptance_regression['report_path']]
...
def checked_manifest():
    ...
    acceptance_regression = checked_acceptance_regression()
    if manifest.get('acceptance_regression')!=acceptance_regression:
        raise RuntimeError('Acceptance regression changed after freeze')
    required = {str(p.relative_to(ROOT)) for p in ROOT.rglob('*.py') if '__pycache__' not in p.parts}
    required |= {'PRESERVED_V317.json','PRESERVED_FIRST_RELEASE.json','EVALUATION.md','evaluation/protocol.json','evaluation/validation_cases.json','evaluation/known_public_tasks.json',
                 'evaluation/validation_selection.json','evaluation/seed_commitment.json',
                 'evaluation/acceptance_regression.json',acceptance_regression['report_path']}
...
def run_evaluation(phase):
    ...
    cases = (holdout_cases() if phase=='holdout' else validation_cases() if phase=='validation' else
             first_release_regression_cases() if phase=='first-release-regression' else regression_cases())
    if not cases:
    ...
    report = {'version': VERSION, 'phase': phase, 'created_utc': now(), 'protocol_digest': digest(public_rules()),
              ...
              'results': summarize(sessions,cases,selection,phase),
              'independence':{'drawn_cases':len(cases),'unique_new_instances':sum(c.get('independent_new_instance',False) for c in cases),
                              'prior_public_overlap_case_ids':[c['id'] for c in cases if c.get('prior_public_overlap')],
                              'within_draw_duplicate_case_ids':[c['id'] for c in cases if c.get('duplicate_of_current_draw')]},
...
def self_test():
    ...
    synthetic = [deepcopy(validation_cases()[0]),deepcopy(validation_cases()[0]),
                 case('UNIQUE_TEST','spectrum',q(amplitude='0',offset='17/19'),'1/10000000000')]
    synthetic[0]['id'],synthetic[1]['id']='DUPLICATE_TEST_1','DUPLICATE_TEST_2'
    classify_draws(synthetic)
    check('Known-task duplicates retained and labelled, never resampled',len(synthetic)==3 and
          synthetic[0]['prior_public_overlap'] and not synthetic[0]['independent_new_instance'] and
          synthetic[1]['duplicate_of_current_draw']=='DUPLICATE_TEST_1' and synthetic[2]['independent_new_instance'])
    with tempfile.TemporaryDirectory(prefix='geometry-release-import-test-') as tmp:
        first_adapter, _, _ = load_adapter('first_release',tmp)
        check('first_release loads its frozen file, not same-name new source',first_adapter.implementation_root==str(FIRST_RELEASE)
              and Path(sys.modules['release_service'].__file__).resolve()==(FIRST_RELEASE/'release_service.py').resolve())
        rejected = False
        try:
            load_adapter('release',tmp)
        except RuntimeError as error:
            rejected = 'contamination' in str(error)
        check('Loading a second release version in the same interpreter is rejected',rejected)
...
def _old_seed_read_is_forbidden():
    for source in (OLD,FIRST_RELEASE):
        try:
            read_json(source/'evaluation/private_seed.json')
        except RuntimeError:
            continue
        return False
    return True
...
def main():
    ...
    parser.add_argument('command',choices=['initialize','self-test','select-validation','run-validation','run-regression','run-first-release-regression','freeze','release-holdout','run-holdout','worker'])
    ...
    if args.command=='initialize': result=initialize()
        ...
        report=run_evaluation(args.command.removeprefix('run-'))
        result={'report_path':report['report_path'],'gates':report['results']['preregistered_gates'],
                'methods':{name:{'robust_successes':value['robust_successes'],'cases':value['cases'],
                                'full_session_wall_seconds':value['full_session_wall_seconds'],
                                'full_session_cpu_seconds':value['full_session_cpu_seconds']}
                           for name,value in report['results']['methods'].items()}}
