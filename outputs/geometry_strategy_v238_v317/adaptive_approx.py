...
class ShapeCache:
    """Explicit bounded, process-local reuse of normalized planning shapes.

    Certificate construction and frozen replay always run independently of it.
    Supplying no cache makes the reported planning cost a cold-run cost.
    """
    def __init__(self, capacity=256):
        pieces._integer(capacity, 1, 4096, 'shape_cache_capacity')
        self.capacity = capacity
        self._values = OrderedDict()

    def get(self, degree, ratio):
        key = (degree, ratio)
    ...
    def put(self, degree, ratio, value):
        key = (degree, ratio)
...
def _shape_error(degree, ratio, cache, cost):
    cost['shape_requests'] = cost.get('shape_requests', 0)+1
    shape = cache.get(degree, ratio) if cache is not None else None
    if shape is not None:
        cost['shape_cache_hits'] = cost.get('shape_cache_hits', 0)+1
        return shape
    shape = F(pieces._cell(ratio, F(1), degree, F(1), F(0))['error_squared_dt'])
    cost['shape_evaluations'] += 1
    if cache is not None:
        cache.put(degree, ratio, shape)
    return shape
...
def _finish(function, tolerance, options, attempts, diagnostics, cost):
    from fast_math import piecewise_model
    certificate = piecewise_model(function, **options)
    ...
    predicted = diagnostics.get('error_decomposition', {}).get('error_squared')
    if predicted is not None and F(predicted) != F(certificate['error_squared']):
        raise ArithmeticError('Planning error did not match the independently rebuilt certificate')
...
def solve(function, tolerance='1/100000000', max_degree=24, max_levels=64, **budget):
    ...
    if set(budget)-{'root_ratio', 'root_ratios', 'sqrt_bits', 'max_sqrt_bits', 'shape_cache',
                   'max_shape_evaluations', 'max_certificate_slots'}:
        raise ValueError('Unknown approximation budget')
    cache = budget.get('shape_cache')
    if cache is not None and type(cache) is not ShapeCache:
        raise ValueError('shape_cache must be a bounded ShapeCache instance')
    ...
    shape_limit = pieces._integer(budget.get('max_shape_evaluations', 264), 0, 264, 'max_shape_evaluations')
    slot_limit = pieces._integer(budget.get('max_certificate_slots', 2113), 2, 2113, 'max_certificate_slots')
    cost = {'shape_evaluations': 0, 'shape_cache_hits': 0, 'shape_requests': 0,
            'level_comparisons': 0, 'planning_cache_supplied': cache is not None}
    ...
    if amplitude == 0:
        options = {'degree': 0, 'levels': 1, 'root_ratio': str(ratios[0]), 'sqrt_bits': bits}
        return _finish(function, tolerance, options, [], {'selection': 'exact_constant',
                       'error_decomposition': {'error_squared': '0'}}, cost)
    # With a maximum dyadic denominator, this is the largest reportable bound
    # not exceeding tolerance. A tiny exact error alone is insufficient.
    denominator = 1 << maximum_bits
    reportable_target = F(tolerance.numerator*denominator//tolerance.denominator, denominator)
    normalized_target = (reportable_target or tolerance)/abs(amplitude)
    attempts, best, best_feasible, exhausted = [], None, None, False
    cost['shape_candidates_pruned_by_cost_bound'] = 0
    cost['core_bound_comparisons'] = 0
    for ratio in ratios:
        core_levels, checks, core_possible, _ = minimum_levels(F(0), normalized_target, ratio, F(1), max_levels)
        cost['core_bound_comparisons'] += checks
        for degree in range(max_degree+1):
            # The nonnegative annulus term can never cancel the core term.
            # Thus every later degree costs at least this many expanded slots.
            if best_feasible is not None and (not core_possible or
                    1+core_levels*(degree+1) >= best_feasible['expanded_coefficient_slots']):
                cost['shape_candidates_pruned_by_cost_bound'] += max_degree-degree+1
                break
            available_levels = min(max_levels, (slot_limit-1)//(degree+1))
            if available_levels == 0:
                break
            if cost['shape_evaluations'] >= shape_limit and (cache is None or cache.get(degree, ratio) is None):
                exhausted = True
                break
            shape = _shape_error(degree, ratio, cache, cost)
            levels, checks, reachable, reason = minimum_levels(shape, normalized_target, ratio, F(1), available_levels)
            if reason == 'core_level_budget' and available_levels < max_levels:
                reason = 'coefficient_slot_budget'
            ...
            attempt['expanded_coefficient_slots'] = 1+levels*(degree+1)
            attempts.append(attempt)
            if best is None or F(attempt['error_squared']) < F(best['error_squared']):
                best = attempt
            if reachable and reportable_target > 0 and (best_feasible is None or
                    attempt['expanded_coefficient_slots'] < best_feasible['expanded_coefficient_slots']):
                best_feasible = attempt
        if exhausted:
            break
    if best is None:
        cost.update({'certificate_builds': 0, 'certificate_replays': 0})
        return {'format': FORMAT, 'function': function, 'tolerance': str(tolerance),
                'status': 'open', 'certificate': None, 'attempts': attempts, 'cost': cost,
                'diagnostics': {'stopping_reason': 'shape_evaluation_budget',
                                'target_proved_unattainable': False}, 'spectral_transfer_claimed': False}
    ...
    return _finish(function, tolerance, options, attempts,
                   {'error_decomposition': best, 'selection': 'minimum_attainable_expanded_slots',
                    'root_ratios_considered': list(map(str, ratios)), 'rounding': rounding,
                    'normalized_tolerance': str(normalized_target),
                    'reportable_target_at_max_bits': str(reportable_target),
                    'minimum_slots_within_configured_family': best_feasible is not None and not exhausted,
                    'optimality_scope': 'Listed root ratios, degrees <= max_degree, levels <= max_levels, '
                                        'one constant core and equal degree on all geometric annuli; '
                                        'expanded slots only, not runtime or arbitrary approximants.',
                    'stopping_reason': 'shape_evaluation_budget' if exhausted else 'planned_search_complete',
                    'shape_evaluation_limit': shape_limit, 'certificate_slot_limit': slot_limit,
                    'offset_and_axis_preserved_in_certificate': True}, cost)
