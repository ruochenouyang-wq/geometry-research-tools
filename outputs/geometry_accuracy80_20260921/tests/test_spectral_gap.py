...
class SpectralGapTests(unittest.TestCase):
    ...
    def test_custom_ground_replays_when_old_eta_rejects(self):
        import power_forms as forms
        q = function('-22/23', '-13/27')
        with self.assertRaises(ValueError):
            g.direct.form_bound(q, 40)
        form = forms.select_form(q, tail_start=13)
        c = g.certified_ground(q, 0, True, 12, 12, 16,
                               form_certificate=form, form_verifier=forms.verify_form)
        self.assertTrue(g.verify_ground(c, q, 0, True, forms.verify_form))
        self.assertFalse(g.verify_ground(c, q, 0, True))
        self.assertLess(g.F(c['lower']), g.F(c['upper']))
        for edit in (
            lambda x: x.update(lower='100'),
            lambda x: x.update(mean_zero=1),
            lambda x: x['form_certificate']['form'].update(energy_factor='1'),
            lambda x: x['form_certificate']['selected_form']['proof'].update(eta_upper='0'),
            lambda x: x['kernel_evidence']['C'][0].__setitem__(0, '0'),
            lambda x: x['lower_inertia'].__setitem__(0, True),
        ):
            bad = copy.deepcopy(c)
            edit(bad)
            self.assertFalse(g.verify_ground(bad, q, 0, True, forms.verify_form))

    def test_default_ground_agrees_with_frozen_sector(self):
        c = g.certified_ground(self.q, 1, True, 6, 8, 16)
        frozen = g.direct.certify_sector(self.q, 1, True, 6, 16, 40, 8)
        expected = dict(frozen, format=g.GROUND_FORMAT, form_certificate=None)
        self.assertEqual(c, expected)
        self.assertTrue(g.verify_ground(c, self.q, 1, True))
