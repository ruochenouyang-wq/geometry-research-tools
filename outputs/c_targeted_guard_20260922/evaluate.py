"""Frozen C3 versus C3G: eight fixed post-result targeted regressions, 3 cold runs.
...
PREVIOUS = ROOT.parent / 'c_targeted_iteration_20260922'
SYSTEMS = ('C3', 'C3G')
REPEATS = 3
SECONDS = 10.0
CASE_COUNT = 8
VERSION = 'c3g_targeted_regression_v1'
SELECTED_IDS = ('E18','N29','N30','E13','E08','E19','E21','E24')
...
def meta(item):
    result = {key: deepcopy(item[key]) for key in ('id', 'dimension', 'label', 'classification') if key in item}
    result['cohort'] = 'previously_revealed_neighbors' if item.get('classification') == 'sealed_neighbor_public' else 'prior_legacy_cases'
    return result
...
def source_paths(root=None):
    ...
    excluded = {'evaluation','notes','review','results','__pycache__'}
    return [p for p in sorted(root.rglob('*.py')) if not excluded.intersection(p.relative_to(root).parts)]


def mathematical_identity(item):
    task = item['task']
    function = deepcopy(task['function'])
    function.pop('axis', None)  # Rotation and tolerance changes are not new mathematical identities.
    return digest({'kind':task['kind'],'function':function,'mean_zero':task.get('mean_zero')})
...
def identity():
    ...
    if pointer != protocol['reference_freeze'] or len(old_files) != protocol['reference_union_file_count']:
        raise RuntimeError('Reference C3 freeze differs from the precommitted identity')
    ...
    new_files = {str(p.resolve()): sha(p) for p in source_paths()}
...
def inputs():
    ...
    expected = {'systems':list(SYSTEMS),'repetitions':REPEATS,'cold_seconds':10,
                'case_count':CASE_COUNT,'fixed_runs':48,'selected_ids':list(SELECTED_IDS)}
    for key,value in expected.items():
        if protocol.get(key)!=value:
            raise ValueError('Protocol contradicts fixed value: '+key)
    if protocol.get('thread_environment')!=THREAD_ENV:
        raise ValueError('Thread controls changed')
    if type(cases) is not list or [c['id'] for c in cases]!=list(SELECTED_IDS):
        raise ValueError('Require exactly the approved ordered eight-case subset')
    if sha(ROOT/'cases.json')!=protocol['cases_sha256']:
        raise ValueError('Committed subset changed')
    original=read(PREVIOUS/'cases.json')
    if sha(PREVIOUS/'cases.json')!=protocol['original_cases_sha256']:
        raise ValueError('Original 30-task list changed')
    by_id={c['id']:c for c in original}
    if cases!=[by_id[name] for name in SELECTED_IDS]:
        raise ValueError('Every selected original task and metadata must be unchanged')
    if sum(c.get('classification')=='expected_unsupported_control' for c in cases)!=1:
        raise ValueError('Fixed denominator is seven mathematical tasks plus one boundary control')
    target=protocol['N29_recovery_standard']
    if exact(target['maximum_width_exact'])<=0 or target['solver_visible'] is not False:
        raise ValueError('Recovery criterion is external assessment only')
    if sha(target['provenance_report'])!=target['provenance_report_sha256']:
        raise ValueError('Original result provenance changed')
    return cases,protocol


def runtime_environment():
    program = '''import contextlib,io,json,os,platform,sys
data={'python_executable':sys.executable,'python_version':sys.version.split()[0],
      'platform':sys.platform,'machine':platform.machine(),
      'thread_environment':{k:os.environ.get(k) for k in ('OPENBLAS_NUM_THREADS','OMP_NUM_THREADS','MKL_NUM_THREADS','VECLIB_MAXIMUM_THREADS','NUMEXPR_NUM_THREADS','BLIS_NUM_THREADS')}}
try:
 import numpy as np
 buf=io.StringIO()
 with contextlib.redirect_stdout(buf):np.show_config()
 data.update(numpy_available=True,numpy_version=np.__version__,numpy_build_config=buf.getvalue())
 try:
  import threadpoolctl
  data['threadpool_info']=threadpoolctl.threadpool_info()
 except ImportError:data['threadpool_info']='threadpoolctl unavailable; requested limits are recorded, not inferred as observed thread counts'
except ImportError as error:data.update(numpy_available=False,numpy_error=str(error))
print(json.dumps(data,sort_keys=True))
'''
    completed = subprocess.run([sys.executable,'-B','-c',program],capture_output=True,text=True,
        check=True,timeout=10,env=dict(os.environ,**THREAD_ENV,PYTHONDONTWRITEBYTECODE='1',PYTHONHASHSEED='0'))
    return json.loads(completed.stdout)


def freeze():
    inputs()
    for name in ('controller.py', 'evaluate.py', 'selftest_evaluate.py'):
    ...
    write(path, {'version': VERSION, 'classification': 'post_result_targeted_regression_not_holdout',
                 'identity': before, 'runtime_environment':environment,
...
def checked_freeze():
    ...
    if frozen['runtime_environment'] != runtime_environment():
        raise RuntimeError('Python/NumPy/BLAS configuration changed after freeze')
...
def provenance(system, response):
    ...
    allowed = ('native_c3','reused_c2','reused_c1','baseline_fallback','c3_guard_baseline_improvement')
    counters = response.get('counters', {})
    counters = counters if type(counters) is dict else {}
    baseline_calls = counters.get('baseline_fallback_calls', 0)
    guard_calls = counters.get('guard_baseline_calls',0)
    reason = response.get('fallback_reason')
    return {'route':route if route in allowed else 'unclassified','solver_reported_route':route,
            'route_recognized':route in allowed,'fallback_reason':reason,
            'route_chain':deepcopy(response.get('route_chain')),
            'guard':deepcopy(response.get('guard')),
            'guard_attempted':response.get('guard',{}).get('attempted') if type(response.get('guard')) is dict else False,
            'guard_baseline_calls':guard_calls,
            'baseline_calls_observed':baseline_calls,
            'baseline_work_observed':route in ('baseline_fallback','c3_guard_baseline_improvement') or bool(baseline_calls) or bool(guard_calls),
            'reuse_reason':response.get('native_kind') if route in ('reused_c1','reused_c2') else None,
            'explanation_complete':route in allowed and (route!='baseline_fallback' or bool(reason))}
...
class Synthetic:
    ...
    def solve(self, task, progress=None):
        ...
        if self.behavior == 'unsupported':
            return {'status': 'unsupported_method_domain', 'certificate': None, 'route': 'native_c3', 'failure_stage': 'capability_check'}
        ...
        return {'certificate': certificate, 'route': 'native_c3', 'synthetic_child_metrics':child_metrics,
...
def load_engine(spec):
    ...
    # Both systems are checked through the same original frozen verifier.
    trusted = load_module(ORIGINAL/'shared_baseline.py','c3_eval_trusted_baseline')
    baseline = trusted.baseline
    def verify(certificate, task):
        return baseline.assess(certificate, baseline.normalize_task(task))
    controller_root = PREVIOUS if spec['system']=='C3' else ROOT
    sys.path.insert(0,str(controller_root))
    module = load_module(controller_root/'controller.py','c3_eval_controller_'+spec['system'])
    return module.solve, verify, True
...
def run_process(spec, destination):
    ...
    child = subprocess.Popen([sys.executable, '-B', str(Path(__file__).resolve()), 'worker',
        ...
        env=dict(os.environ, **THREAD_ENV, PYTHONDONTWRITEBYTECODE='1', PYTHONHASHSEED='0'))
    stderr.close()
...
def summarize(rows, cases):
    ...
    for system in SYSTEMS:
        ...
        grouped = {}
        for field in ('classification','cohort','dimension','kind'):
    ...
    for system in SYSTEMS:
        result[system]['both_systems_stable_common_subset'] = common
...
def regressions(rows,cases):
    result={'stable_success_lost':[],'stable_success_gained':[],'paired_run_success_regressions':[],
            'reuse_and_fallback_rows':[],'same_success_cost_increases':[]}
    for case in cases:
        paired={s:sorted([r for r in rows if r['system']==s and r['case']['id']==case['id']],key=lambda r:r['repetition']) for s in SYSTEMS}
        stable={s:len(paired[s])==REPEATS and all(r['success'] for r in paired[s]) for s in SYSTEMS}
        if stable['C3'] and not stable['C3G']:result['stable_success_lost'].append(meta(case))
        if stable['C3G'] and not stable['C3']:result['stable_success_gained'].append(meta(case))
        for old,new in zip(paired['C3'],paired['C3G']):
            if old['success'] and not new['success']:
                result['paired_run_success_regressions'].append({**meta(case),'repetition':new['repetition'],
                    'failure_class':new.get('failure_class'),'failure_stage':new.get('failure_stage'),
                    'failure_reason':new.get('failure_reason'),'last_internal_action':new.get('last_internal_action'),
                    'raw_directory':new['raw_directory']})
        for row in paired['C3G']:
            route=row['provenance']['route']
            if route in ('reused_c2','reused_c1','baseline_fallback') or row['provenance'].get('baseline_work_observed'):
                result['reuse_and_fallback_rows'].append({**meta(case),'repetition':row['repetition'],
                    'provenance':row['provenance'],'success':row['success'],'raw_directory':row['raw_directory']})
        if all(stable.values()):
            for metric in ('wall_seconds','cpu_seconds','peak_rss_bytes'):
                old=statistics.median(r['process'][metric] for r in paired['C3'])
                new=statistics.median(r['process'][metric] for r in paired['C3G'])
                if new>old:
                    result['same_success_cost_increases'].append({**meta(case),'metric':metric,
                        'C3_median':old,'C3G_median':new,'ratio':new/old if old else None,
                        'interpretation':'observed three-repeat increase, not a significance claim'})
    return result


def guard_validation(rows,protocol):
    threshold=exact(protocol['N29_recovery_standard']['maximum_width_exact'])
    details=[]
    for row in rows:
        if row['case']['id']=='N29':
            assessment=row.get('assessment',{})
            valid=assessment.get('certificate_valid') is True and row['within_budget']
            metric=assessment.get('metric')
            details.append({'system':row['system'],'repetition':row['repetition'],
                'complete_valid_certificate_within_budget':valid,'width_exact':metric,
                'restores_previous_C2_precision':valid and metric is not None and exact(metric)<=threshold,
                'original_tolerance_met':row['success'],'raw_directory':row['raw_directory']})
    paired=[]
    for name in SELECTED_IDS:
        old=sorted([r for r in rows if r['system']=='C3' and r['case']['id']==name],key=lambda r:r['repetition'])
        new=sorted([r for r in rows if r['system']=='C3G' and r['case']['id']==name],key=lambda r:r['repetition'])
        for a,b in zip(old,new):
            av=a.get('assessment',{});bv=b.get('assessment',{})
            both_valid=av.get('certificate_valid') is True and bv.get('certificate_valid') is True and a['within_budget'] and b['within_budget']
            paired.append({'id':name,'repetition':b['repetition'],'both_certificates_valid_within_budget':both_valid,
                'C3_width_or_error':av.get('metric'),'C3G_width_or_error':bv.get('metric'),
                'metric_regression':both_valid and exact(bv['metric'])>exact(av['metric']),
                'lost_valid_certificate':av.get('certificate_valid') is True and a['within_budget'] and not(bv.get('certificate_valid') is True and b['within_budget']),
                'original_target_regression':a['success'] and not b['success']})
    return {'N29_standard':deepcopy(protocol['N29_recovery_standard']),'N29_rows':details,
        'paired_precision_preservation':paired,
        'note':'Recovery is an external quality criterion and never counts as original tolerance success.'}


def system_order(repetition,original_case_index):
    shift=(repetition+original_case_index)%len(SYSTEMS)
...
def run():
    ...
    frozen_environment = read(pointer['manifest'])['runtime_environment']
    before = identity()
    destination = ROOT/'evaluation'/('final_'+stamp())
    destination.mkdir(parents=True)
    write(destination/'fixed_subset.json',{'freeze':pointer,'cases':cases,
        'classification':'post_result_targeted_regression_not_holdout','new_tasks':False})
    write(destination/'started.json', {'version':VERSION,'freeze':pointer,'identity_before':before,'cases_sha256':digest(cases)})
    rows = []
    original_positions={case['id']:i for i,case in enumerate(cases)}
    for repetition in range(REPEATS):
        case_order = cases[repetition%len(cases):]+cases[:repetition%len(cases)]
        for case_number,case in enumerate(case_order):
            order=system_order(repetition,original_positions[case['id']])
            for system in order:
    ...
    try:
        ...
        after=None;unchanged=False;error=type(exception).__name__+': '+str(exception)
    ...
    report = {'version':VERSION,'classification':'post_result_targeted_regression_not_holdout','complete_fixed_denominator':complete,
        'valid_frozen_comparison':complete and unchanged,'freeze':pointer,'identity_before':before,'identity_after':after,
        'identity_error':error,'rows':rows,'summary':summarize(rows,cases),'regressions':regressions(rows,cases),
        'guard_validation':guard_validation(rows,protocol),
        ...
        'limitations':['Three serial interleaved repeats on a shared machine; timing is preliminary.',
            ...
            'Eight tasks selected after observing a precision regression; this is targeted regression, not new holdout or all-30 performance.',
            'All guard attempts and old baseline work are charged; interval improvement is separate from original tolerance success.',
