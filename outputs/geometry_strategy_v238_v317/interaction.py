...
def offline_text_counter(cache_dir, *, _native_only=False):
    ...
    Use a compatible installed encoder or vendored native extensions. If their
    ABI differs from this Python, try an existing local Python in an isolated,
    persistent worker. Neither path installs dependencies or accesses the network.
    """
    ...
    compatible_vendor = all(any((dependency/package/(basename+suffix)).is_file()
                               for suffix in importlib.machinery.EXTENSION_SUFFIXES)
                            for package,basename in [('tiktoken','_tiktoken'),('regex','_regex')])
    old_path = list(sys.path)
    previous = os.environ.get('TIKTOKEN_CACHE_DIR')
    try:
        if compatible_vendor and str(dependency) not in sys.path:
        ...
        def native_counter(text):
            return {name:len(encoder.encode(text,disallowed_special=()))
                    for name,encoder in encoders.items()}
        native_counter.measurement_runtime = {'strategy':'in_process','executable':sys.executable,
                                               'python_version':sys.version.split()[0],
                                               'tiktoken_version':tiktoken.__version__}
        return native_counter
    except (ImportError,RuntimeError,OSError) as error:
        native_failure = type(error).__name__+': '+str(error)
    finally:
        sys.path[:] = old_path
        if previous is None:
    ...
    if _native_only:
        raise RuntimeError('No compatible native offline encoder: '+native_failure)
...
try:
    import interaction
    counter=interaction.offline_text_counter(sys.argv[2],_native_only=True)
    print(json.dumps({'ready':True,'runtime':counter.measurement_runtime}),flush=True)
except Exception as error:
    print(json.dumps({'ready':False,'reason':type(error).__name__+': '+str(error)}),flush=True)
    raise SystemExit(1)
for line in sys.stdin:
    try:
        value=json.loads(line)
        if not isinstance(value,str):raise ValueError('Expected text')
        print(json.dumps({'ok':True,'counts':counter(value)}),flush=True)
    except Exception as error:
        print(json.dumps({'ok':False,'reason':type(error).__name__+': '+str(error)}),flush=True)
'''
    ...
    def read_line(process, timeout=30):
        with selectors.DefaultSelector() as selector:
            selector.register(process.stdout,selectors.EVENT_READ)
            if not selector.select(timeout):
                raise RuntimeError('Local offline encoder timed out')
        line = process.stdout.readline()
        if not line:
            raise RuntimeError('Local offline encoder exited before returning counts')
        return json.loads(line)

    candidates = [shutil.which('python3'), shutil.which('python3.9'),
                  '/usr/bin/python3','/Library/Developer/CommandLineTools/usr/bin/python3']
    tried = set()
    failures = [native_failure]
    for executable in candidates:
        if not executable or not Path(executable).is_file():
            continue
        identity = str(Path(executable).resolve())
        if identity in tried or identity == str(Path(sys.executable).resolve()):
            continue
        tried.add(identity)
        process = None
        try:
            process = subprocess.Popen([executable,'-I','-B','-u','-c',worker_code,str(root),str(cache_dir)],
                stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.PIPE,
                text=True,encoding='utf-8')
            ready = read_line(process)
            if ready.get('ready') is not True:
                raise RuntimeError(ready.get('reason','Local encoder could not initialize'))
            ...
            continue
        lock = threading.Lock()
        def worker_counter(text, process=process, lock=lock):
            if not isinstance(text,str):
                raise ValueError('Token counter accepts text only')
            with lock:
                if process.poll() is not None:
                    raise RuntimeError('Local offline encoder is closed')
                try:
                    process.stdin.write(json.dumps(text,ensure_ascii=False)+'\n')
                    process.stdin.flush()
                    answer = read_line(process)
                except (OSError,ValueError,RuntimeError):
                    stop(process)
                    raise
                if answer.get('ok') is not True:
                    raise RuntimeError(answer.get('reason','Local offline count failed'))
                counts = answer.get('counts')
                if (not isinstance(counts,dict) or set(counts) != {'o200k_base','cl100k_base'} or
                        any(type(value) is not int or value < 0 for value in counts.values())):
                    raise RuntimeError('Invalid counts from local offline encoder')
                return counts
        worker_counter.measurement_runtime = {**ready['runtime'],'strategy':'local_subprocess',
                                               'host_python_version':sys.version.split()[0]}
        worker_counter.close = lambda process=process:stop(process)
        atexit.register(stop,process)
        return worker_counter
    raise RuntimeError('No compatible local offline encoder. '+ '; '.join(failures))
