...
def solve(function, tolerance='1/100000000', mean_zero=False, max_terms=10,
    ...
    form = form_certificate(q)
    gap = spectral_gap.certified_gap(q, m=0, mean_zero=False, modes=gap_modes,
    ...
    for count in stages:
        if perf_counter()-started >= wall_seconds:
            ...
            break
        begin = perf_counter()
        row = {'terms': count, 'verified': False}
        try:
    ...
    return {'status': best['status'] if best is not None else 'unsupported',
            ...
            'certificate': best, 'attempts': attempts, 'stop_reason': reason,
            'form_certificate': form, 'elapsed_seconds': elapsed,
            'within_budget': elapsed <= wall_seconds,
            'budget': {'max_terms': max_terms, 'wall_seconds': wall_seconds,
