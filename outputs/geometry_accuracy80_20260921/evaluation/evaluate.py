...
def public_catalog():
    ...
    static_path = DATA/'public_test_cases.json'
    if static_path.exists():
        result.extend(read_json(static_path))
    # A revealed batch is public forever, even if formal measurement crashed.
    for path in sorted((DATA/'batches').glob('B*/holdout_cases.json')):
        for item in read_json(path):
            result.append({'source': str(path), 'case_id': item['id'], 'task': item['task']})
    return result
...
def source_identity():
    ...
    roots = [p for p in ROOT.parent.glob('geometry_*') if p.is_dir()]
    files = {}
    for root in roots:
