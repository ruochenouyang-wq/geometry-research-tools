...
class FastMathTests(unittest.TestCase):
    ...
    def test_gram_prefix_exact_and_immutable(self):
        for ss in (enriched.DEFAULT_EXPONENTS[:n] for n in range(1,11)):
            new = fast.trial_matrices(ss)
            self.assertEqual(new,enriched.trial_matrices(ss))
            v = [F((-1)**i,i+1) for i in range(len(ss))]
            self.assertEqual(fast.trial_statistics(ss,v),enriched.trial_statistics(ss,v))
        with self.assertRaises(TypeError): new[0][0][0] = F(77)
        self.assertEqual(fast.counters()['gram_prefix_extensions'],10)
        ss = [F(0),F(8,5),F(3),F(6)]
        self.assertEqual(fast.trial_matrices(ss),enriched.trial_matrices(ss))
        for ss in ([0,1],[2,0],[0,0]):
            with self.assertRaises(ValueError): fast.trial_matrices(ss)

    def test_incremental_ldl_reconstruction_solve_and_proposal(self):
        for n in (1,3,6,10):
            ss = enriched.DEFAULT_EXPONENTS[:n]
            H = fast.trial_matrices(ss)[1]
            L,D = fast._ldl_factors(ss)
            rebuilt = tuple(tuple(sum((L[i][k]*D[k]*L[j][k] for k in range(n)),F(0))
                                  for j in range(n)) for i in range(n))
            self.assertEqual(rebuilt,H)
            rhs = [F((-1)**i,i+1) for i in range(n)]
            self.assertEqual(fast._solve_factors((L,D),rhs),enriched._ldl_solver(H)(rhs))
            for iterations in (0,3):
                new = fast.propose_trial(ss,32,iterations)
                old = enriched.propose_trial(ss,32,iterations)
                new.pop('elapsed_seconds'); old.pop('elapsed_seconds')
                self.assertEqual(new,old)
        self.assertEqual(fast.counters()['ldl_new_pivots'],10)
        self.assertEqual(fast.counters()['ldl_new_offdiagonal_entries'],45)
        with self.assertRaises(ValueError): fast.propose_trial([0],31,3)

    def test_fused_statistics_signed_sparse_and_invalid(self):
        ss = enriched.DEFAULT_EXPONENTS[:10]
        for v in ([F(1)]*10,[F((-1)**i,i+1) for i in range(10)],
                  [F(i == 0) for i in range(10)],
                  [F(1,2**112) if i%2 else F(-1,3) for i in range(10)]):
            self.assertEqual(fast.trial_statistics(ss,v),enriched.trial_statistics(ss,v))
        for v in ([0]*10,[1]*9,[True]*10,[0.5]*10):
            with self.assertRaises(ValueError): fast.trial_statistics(ss,v)
        with self.assertRaises(ValueError): fast.MomentTable(Q).integral(1,1,2,3)

    def test_complete_enrich_matches_frozen_search_and_replay(self):
        source = source_fixture()
        new = fast.enrich(source,max_terms=6,precision_bits=112,iterations=14)
        old = enriched.enrich(source,max_terms=6,precision_bits=112,iterations=14)
        self.assertEqual(new['certificate'],old['certificate'])
        self.assertEqual([r['exact_width'] for r in new['attempts']],
                         [r['exact_width'] for r in old['attempts']])
        self.assertTrue(enriched.verify(new['certificate'],source))
