...
def proposal(function, powers, precision_bits=112, iterations=14, **options):
    ...
    shift_policy = options.pop('shift_policy', 'none')
    ...
    if transform is not None or adaptive or shift_policy == 'certified':
    ...
    if residual_steps:
        refined = residual_refinement(q,ss,result['coefficients'],precision_bits,residual_steps,transform)
...
def solve(function, tolerance='1/100000000', max_terms=16,
    ...
    proposal_options = {'leading_cancellation': budget.get('leading_cancellation', False),
                        ...
                        'shift_policy': budget.get('shift_policy', 'none'), 'source': source}
    for n in counts:
        begin = perf_counter()
