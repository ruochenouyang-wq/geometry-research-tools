"""C2 strong full-space search with proved coarse gaps and bounded candidates.
...
VERSION = 'c2-proved-coarse-gap-fullspace'
_LOADED = None
...
def _modules():
    ...
    if _LOADED is None:
        ...
        spec = importlib.util.spec_from_file_location('_c2_fullspace_frozen_solver',FROZEN/'solver.py')
...
def _assemble_best(best,q,tolerance,run,permit_expired=False):
    ...
    cert=run.action('recover_best_certificate' if permit_expired else 'assemble_original_certificate',
        lambda:full.certificate(q,candidate['powers'],candidate['coefficients'],gap,tolerance),
        permit_expired=permit_expired)
    cert=json.loads(json.dumps(cert,allow_nan=False))
...
def solve(raw_task,*,wall_seconds=None,progress=None):
    ...
    try:
        ...
        for modes in (12,16):
            ...
            for terms in ((6,10,14,16) if modes==12 else (16,)):
        ...
        if best is not None:
            # The original certificate builder checks the gap and recomputes
            # the exact full residual. Task-bound independent replay belongs
            # to the common controller, not a duplicate component assessment.
