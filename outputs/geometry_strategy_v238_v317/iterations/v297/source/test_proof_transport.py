...
class TransportTests(unittest.TestCase):
    ...
    def test_expected_problem_space_and_accuracy_bindings(self):
        cert = pt.constant_spectrum('1/3')
        self.assertTrue(pt.verify(cert, expected_function=cert['function'], expected_mean_zero=True,
                                  expected_tolerance='1/1000000000000'))
        self.assertFalse(pt.verify(cert, expected_function=dict(cert['function'], axis=0)))
        self.assertFalse(pt.verify(cert, expected_mean_zero=False))
        self.assertFalse(pt.verify(cert, expected_mean_zero=1))
        self.assertFalse(pt.verify(cert, expected_tolerance=0))
        source = pt.backend.pieces.piecewise_model({'kind': 'axis_profile', 'profile': 'abs_power',
                                                  'exponent': '-1/4'}, levels=2, degree=1)
        approximation = pt.approximation_axis(source, 0)
        self.assertTrue(pt.verify(approximation, expected_tolerance=source['error_upper']))
        self.assertFalse(pt.verify(approximation, expected_tolerance=str(F(source['error_upper'])/2)))
        self.assertFalse(pt.verify(approximation, expected_mean_zero=False))

    def test_constant_keeps_original_abs_power_binding(self):
        q = {'kind': 'axis_profile', 'profile': 'abs_power', 'amplitude': '0',
             'exponent': '-1/3', 'offset': '2/7', 'axis': 1}
        cert = pt.constant_for_function(q, False)
        self.assertTrue(pt.verify(cert, expected_function=q, expected_mean_zero=False))
        self.assertEqual(cert['function']['profile'], 'abs_power')
        self.assertEqual(cert['lower'], '2/7')
        with self.assertRaises(ValueError):
            pt.constant_for_function(dict(q, amplitude='1'))

    def test_invalid_transport_payloads_and_cycles(self):
        cert = pt.spectral_shift(pt.constant_spectrum(), 1)
        bad = deepcopy(cert); bad['parameters']['unproved'] = True
        self.assertFalse(pt.verify(bad))
        bad = deepcopy(cert); bad['sources'][0]['lower'] = '99'
        self.assertFalse(pt.verify(bad))
        bad = deepcopy(cert); bad['sources'] = [bad]
        self.assertFalse(pt.verify(bad))
        bad = deepcopy(cert); bad['sources'] = [cert]*(pt.MAX_SOURCES+1)
        self.assertFalse(pt.verify(bad))
        for invalid_axis in (True, -1, 3, '0'):
            with self.assertRaises(ValueError):
                pt.spectral_axis(pt.constant_spectrum(), invalid_axis)
        with self.assertRaises(ValueError):
            pt.approximation_shift(pt.constant_spectrum(), 1)
        with self.assertRaises(ValueError):
            pt.spectral_shift(pt.constant_spectrum(), 1.25)

    def test_extension_requires_a_mathematical_verifier(self):
        source = pt.constant_spectrum('2/3')
        external = {k: v for k, v in source.items() if k not in ('operation', 'sources', 'parameters')}
        external['format'] = 'test_exact_constant_extension'
        def custom_verifier(cert):
            return cert == external and F(cert['lower']) == F(8, 3) and cert['upper'] == cert['lower']
        cert = pt.spectral_shift(external, 1, verifier=custom_verifier)
        self.assertTrue(pt.verify(cert, verifier=custom_verifier))
        self.assertFalse(pt.verify(cert))
        self.assertFalse(pt.verify(cert, verifier=lambda c: False))
        cert['sources'][0]['lower'] = '999'
        self.assertFalse(pt.verify(cert, verifier=custom_verifier))
