...
def solve(request, solvers=None, verifier=None):
    ...
    best, best_score, best_diagnosis, stale = None, None, None, 0
    seen = set()
    ...
    for _ in range(max_attempts):
        ...
        route = action['route']
        old_route_score = route_scores.get(route)
        meaningful = old_route_score is None or score < old_route_score*(1-min_gain)
        route_stale[route] = 0 if meaningful else route_stale.get(route, 0)+1
        route_scores[route] = min(score, old_route_score) if old_route_score is not None else score
        ...
        if pending:
            ...
            reason = 'no_meaningful_gain' if any(v >= patience for v in route_stale.values()) \
                else 'no_supported_refinement'
            break
    return {'certificate': best, 'attempts': attempts,
            'status': 'target_met' if reason == 'target_met' else (
                'certified_open' if best is not None else reason),
            'stop_reason': reason, 'diagnosis': best_diagnosis or diagnosis,
