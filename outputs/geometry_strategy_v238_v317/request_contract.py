...
def execute(request, executor=None, verifier=None):
    ...
    try:
        ...
        _finite_values(result)
    except Exception as error:
        return failure_response(error, 'executor', request)
    if result.get('ok') is not True:
        return failure_response(RuntimeError(str(result.get('reason', 'Backend did not report successful execution'))),
                                'executor', request, code='backend_failure')
    ...
    try:
        ...
        return failure_response(error, 'verifier', request)
...
def failure_response(error, stage, request=None, code=None):
    """An execution failure is unknown mathematics, never a disproof."""
    if code is None:
        if isinstance(error, ContractError) and stage == 'request':
            code = error.code
        elif isinstance(error, (TimeoutError, MemoryError, RecursionError)):
            code = 'resource_limit'
        elif stage == 'executor' and isinstance(error, ValueError):
            code = 'backend_rejected'
        else:
            code = 'internal_error'
    result = {'ok': False, 'execution_ok': False, 'certificate_valid': None,
              'verified': False, 'bindings_match': None, 'binding_complete': False,
              'target_met': None, 'certificate_target_met': None,
              'status': 'request_rejected' if stage == 'request' and isinstance(error, ContractError) else 'execution_failed',
              'reason_code': code, 'stage': stage, 'error': type(error).__name__,
              'reason': str(error)}
    if isinstance(error, ContractError) and error.field is not None:
        result['field'] = error.field
    if request is not None:
        result['request_digest'] = request_digest(request)
        if 'request_id' in request:
            result['request_id'] = request['request_id']
    return result
...
def process_lines(lines, executor=None, verifier=None):
    for number, line in enumerate(lines, 1):
        ...
        try:
            ...
            stage = 'execution'
            result = execute(request, executor=executor, verifier=verifier)
        except Exception as error:
            result = failure_response(error, stage, request)
...
if __name__ == '__main__':
    import sys
    for response in process_lines(sys.stdin):
        print(json.dumps(response, ensure_ascii=False, sort_keys=True, allow_nan=False), flush=True)
