"""Frozen C2 versus C3: 24 retained tasks plus six precommitted neighbors, 3 cold runs.
...
PREVIOUS = ROOT.parent / 'c_demand_extension_20260922'
SYSTEMS = ('C2', 'C3')
REPEATS = 3
SECONDS = 10.0
CASE_COUNT = 30
VERSION = 'c3_targeted_public_v1'
...
def meta(item):
    ...
    result['cohort'] = 'added_sealed_neighbors' if item.get('classification') == 'sealed_neighbor_public' else 'retained_C2_cases'
    return result
...
def identity():
    ...
    if pointer != protocol['reference_freeze'] or len(old_files) != protocol['reference_union_file_count']:
        raise RuntimeError('Reference C2 freeze differs from the precommitted identity')
...
def inputs():
    ...
    if type(cases) is not list or len(cases) != CASE_COUNT or len({c['id'] for c in cases}) != CASE_COUNT:
        raise ValueError('Require exactly 30 uniquely named fixed cases')
    ...
    if protocol.get('thread_environment') != THREAD_ENV:
        raise ValueError('Thread-control environment differs from the predeclared protocol')
    ...
    if sha(ROOT/'cases.json') != protocol['cases_sha256']:
        raise ValueError('Committed complete case list changed')
    original = read(PREVIOUS/'cases.json')
    if sha(PREVIOUS/'cases.json') != protocol['legacy_cases_sha256'] or cases[:24] != original:
        raise ValueError('All 24 frozen C2 cases must remain exactly unchanged')
    added = cases[24:]
    if len(added)!=6 or digest(added)!=protocol['added_cases_commitment_sha256']:
        raise ValueError('Sealed six-case commitment changed')
    known = {mathematical_identity(c) for c in original}
    added_ids = {mathematical_identity(c) for c in added}
    if len(added_ids)!=6 or known.intersection(added_ids):
        raise ValueError('Added cases must be six distinct nonlegacy mathematical identities')
    if {d:sum(c['dimension']==d for c in added) for d in protocol['new_strata_counts']} != protocol['new_strata_counts']:
        raise ValueError('Sealed stratum counts changed')
    for case in added:
        task,q=case['task'],case['task']['function']
        a,alpha=exact(q['amplitude']),exact(q['exponent'])
        if q['profile']!='abs_power' or not -Fraction(1,2)<alpha<0 or type(q['axis']) is not int or q['axis'] not in (0,1,2):
            raise ValueError('Sealed neighbor is outside the predeclared supported parameter domain')
        if exact(q['offset'])!=0 or task['tolerance'] not in ('1/1000000','1/100000000'):
            raise ValueError('Unexpected sealed offset or precision')
        variance=a*a*alpha*alpha/((1+2*alpha)*(1+alpha)**2)
        category=case['dimension']
        if category=='weak_negative_fullspace' and not(a<0 and task['mean_zero'] is False and variance<1):
            raise ValueError('Weak-negative stratum mismatch')
        if category=='strong_negative_fullspace' and not(a<0 and task['mean_zero'] is False and variance>=1):
            raise ValueError('Strong-negative stratum mismatch')
        if category=='positive_mean_zero' and not(a>0 and task['mean_zero'] is True):
            raise ValueError('Positive mean-zero stratum mismatch')
    positives=[c['task']['function'] for c in added if c['dimension']=='positive_mean_zero']
    if len({q['axis'] for q in positives})!=2 or len({q['amplitude'] for q in positives})!=2 or len({q['exponent'] for q in positives})!=2:
        raise ValueError('The positive controls require distinct axes, exponents and amplitudes')
    for case in cases:
        task = case['task']
        ...
        if case.get('classification') not in ('new_public', 'old_failure_regression', 'expected_unsupported_control', 'sealed_neighbor_public'):
...
def provenance(system, response):
    ...
    allowed = ('native_c2','reused_c1','baseline_fallback') if system=='C2' else ('native_c3','reused_c2','reused_c1','baseline_fallback')
    ...
    return {'route':route if route in allowed else 'unclassified','solver_reported_route':route,
            ...
            'baseline_work_observed':route=='baseline_fallback' or bool(baseline_calls),
...
def load_engine(spec):
    ...
    controller_root = PREVIOUS if spec['system']=='C2' else ROOT
...
def regressions(rows,cases):
    ...
    for case in cases:
        ...
        if stable['C2'] and not stable['C3']:result['stable_success_lost'].append(meta(case))
        if stable['C3'] and not stable['C2']:result['stable_success_gained'].append(meta(case))
        for old,new in zip(paired['C2'],paired['C3']):
        ...
        if all(stable.values()):
            for metric in ('wall_seconds','cpu_seconds','peak_rss_bytes'):
                old=statistics.median(r['process'][metric] for r in paired['C2'])
                new=statistics.median(r['process'][metric] for r in paired['C3'])
                if new>old:
                    result['same_success_cost_increases'].append({**meta(case),'metric':metric,
                        'C2_median':old,'C3_median':new,'ratio':new/old if old else None,
...
def run():
    ...
    # The first public parameter disclosure happens only after verified source freeze.
    write(destination/'revealed_neighbors.json',{'freeze':pointer,
        'cases_commitment_sha256':protocol['cases_sha256'],
        'added_cases_commitment_sha256':protocol['added_cases_commitment_sha256'],
        'cases':cases[24:],'classification':'precommitted_new_parameters_in_known_public_families',
        'isolation':'shared directory discipline, not filesystem permissions'})
    ...
    report = {'version':VERSION,'classification':'public_development_not_holdout','complete_fixed_denominator':complete,
        ...
        'limitations':['Three serial interleaved repeats on a shared machine; timing is preliminary.',
            ...
            'C3 native, reused C2, reused C1 and true baseline fallback are distinct routes.',
