...
class EvidenceTests(unittest.TestCase):
    def test_real_transport_certificates_summarize(self):
        import proof_transport
        certificate = proof_transport.constant_spectrum('1/3',mean_zero=False)
        request = {'function':certificate['function'],'mean_zero':False,'tolerance':'1/100'}
        summary = subject.summarize_result({'certificate':certificate},request,
                                            verifier=proof_transport.verify)
        self.assertEqual(summary['status'],'certified_met')
        self.assertEqual(summary['bounds']['lower'],'1/3')
        path = Path(__file__).resolve().parent.parent/'geometry_precision_followup'/'approximations'/'step_exact.json'
        certificate = proof_transport.approximation_scale(json.loads(path.read_text()),'3/2')
        summary = subject.summarize_result({'certificate':certificate},verifier=proof_transport.verify)
        self.assertEqual(summary['status'],'certified_bound')
...
class TextInteractionTests(unittest.TestCase):
    def test_bound_summary_roundtrip_and_wrong_context_rejected(self):
        cert = example_certificate()
        request = {'function':cert['function'],'mean_zero':True,'tolerance':'1/10'}
        summary = subject.present_result({'certificate':cert},request,lambda c:True,
                                           store=lambda k,c:'memory:'+k)
        bound = subject.bind_summary(summary,request,'r1')
        self.assertEqual(subject.expand_bound_summary(bound,request,'r1'),summary)
        with self.assertRaises(ValueError):
            subject.expand_bound_summary(bound,dict(request,mean_zero=False),'r1')
        with self.assertRaises(ValueError):
            subject.expand_bound_summary(bound,request,'other')
        self.assertIn('protocol',bound)

    def test_count_all_items_and_missing_encoder_is_unknown(self):
        # Stub counts exercise aggregation only; never published as token counts.
        counter = lambda text:{'stub':len(text)}
        measured = subject.measure_conversation([{'text':'failure'},{'text':'detail'}],counter,'rules','schema')
        self.assertEqual(measured['encodings']['stub'],len('rules')+len('schema')+len('failure')+len('detail'))
        self.assertIsNone(measured['actual_api_usage'])
        self.assertFalse(subject.measure_conversation([])['available'])
        self.assertIsNone(subject.measure_conversation([])['encodings'])

    def test_offline_counter_refuses_missing_cache_before_loading(self):
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaises(RuntimeError):
                subject.offline_text_counter(directory)

    def test_local_runtime_exact_counts_and_no_path_pollution(self):
        import sys
        import os
        cache = Path(__file__).resolve().parent/'token_cache'
        if not cache.is_dir():
            self.skipTest('Public encoding cache not present in this checkout')
        previous_path = list(sys.path)
        previous_cache = os.environ.get('TIKTOKEN_CACHE_DIR')
        counter = subject.offline_text_counter(cache)
        try:
            self.assertEqual(counter(''),{'o200k_base':0,'cl100k_base':0})
            self.assertEqual(counter('hello world'),{'o200k_base':2,'cl100k_base':2})
            self.assertEqual(counter('几何研究：误差 ≤ 1/100000000。'),
                             {'o200k_base':14,'cl100k_base':20})
            self.assertEqual(counter('你好，世界'),{'o200k_base':3,'cl100k_base':6})
            self.assertIn(counter.measurement_runtime['strategy'],['in_process','local_subprocess'])
            self.assertEqual(counter.measurement_runtime['tiktoken_version'],'0.11.0')
        finally:
            if hasattr(counter,'close'):
                counter.close()
        self.assertEqual(sys.path,previous_path)
        self.assertEqual(os.environ.get('TIKTOKEN_CACHE_DIR'),previous_cache)

    def test_without_encoder_use_standalone_baseline(self):
        summary = subject.summarize_result({'certificate':example_certificate()})
        selected = subject.choose_response(summary,{},'r1')
        self.assertEqual(selected['strategy'],'standalone')
        self.assertIsNone(selected['comparison'])

    def test_real_token_comparison_includes_protocol_and_details(self):
        cache = Path(__file__).resolve().parent/'token_cache'
        try:
            counter = subject.offline_text_counter(cache)
        ...
        for known in (False,True):
            selected = subject.choose_response(summary,request,'r1',counter,known)
            baseline = selected['comparison']['standalone']
            chosen = counter(subject.canonical(selected['response']))
            for encoding in baseline:
                self.assertLessEqual(chosen[encoding],baseline[encoding])
        events = [{'text':subject.canonical(request)}, {'text':subject.canonical(summary)},
                  {'text':'fetch evidence'}, {'text':subject.canonical(cert)}]
        report = subject.measure_conversation(events,counter,'instructions','schema')
        self.assertEqual(report['items'],6)
        self.assertEqual(report['encodings'],{name:sum(counter(text)[name] for text in
                         ['instructions','schema']+[e['text'] for e in events]) for name in report['encodings']})
