"""Synthetic evaluator checks only: no C2 mathematical search or tuning."""
...
def main():
    ...
    for behavior in ('echo','raise','unsupported','status_only','swallow','memory_timeout','verify_timeout','child_busy','child_group_timeout'):
        ...
        record=e.run_process({'system':'C2','case':case(),'repetition':0,'synthetic':behavior,
    ...
    tests['reused_C1_is_not_unexplained_fallback']=e.provenance('C2',{'route':'reused_c1','native_kind':'existing_demand_threshold','fallback_reason':None})['explanation_complete']
    tests['true_C1_fallback_identified']=e.provenance('C1',{'route':'unchanged_baseline_route','counters':{'baseline_fallback_calls':1}})['route']=='baseline_fallback'
    ...
    for system in e.SYSTEMS:
        for item in cases:
            for repetition in range(e.REPEATS):
                ...
                if system=='C1' and item['id']=='Y':row['success']=False
                fake.append(row)
    summary=e.summarize(fake,cases)
    tests['fixed_denominator_retained']=summary['C1']['run_denominator']==9 and summary['C1']['returned_rows']==9
    tests['control_rejection_not_math_success']=summary['C2']['expected_unsupported_control_outcomes']['reported_unsupported_within_deadline']==3 and summary['C2']['expected_unsupported_control_outcomes']['math_successes']==0
    tests['new_and_regression_separate']=summary['C2']['by_classification']['new_public']['case_denominator']==1 and summary['C2']['by_classification']['old_failure_regression']['case_denominator']==1
    tests['pairwise_same_success_only']=summary['C2']['pairwise_comparisons']['C1']['stable_same_success_subset']==['X']
    tests['math_denominator_excludes_controls']=summary['C2']['mathematical_tasks_excluding_method_boundary_controls']['fixed_case_denominator']==2
