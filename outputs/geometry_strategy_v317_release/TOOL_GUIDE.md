...
这是已经完成的 v238–v317 的发布修订。保留原数学方法和所有证书格式，改进重复检查的开销。**证书**是保存原题、上下界和复查数据的完整记录；**重放**是重新检查这些记录，不必重新寻找候选。
...
```sh
cd /PROJECT
/PYTHON/bin/python3 -B outputs/geometry_strategy_v317_release/release_service.py <<'JSONL'
...
用于生成候选的模块和验证依赖在服务私有实例中接合，不替换共享数学后端的全局函数。每个新证据的首次检查继续使用冻结的验证方法；发布评测还会在缓存之外用冻结验证器独立重放。
