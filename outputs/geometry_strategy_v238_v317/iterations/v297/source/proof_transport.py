...
MAX_TRANSPORT_DEPTH = 32
MAX_TRANSPORT_NODES = 256
MAX_SOURCES = 32
...
def _same_target(claims, kind):
    if not claims or any(c['kind'] != kind for c in claims):
        raise ValueError('All sources must have the required conclusion kind')
    first = claims[0]
    if any(c['function'] != first['function'] or c['scope'] != first['scope'] for c in claims):
        raise ValueError('Source functions and mathematical spaces must agree exactly')
    return first
...
def _derive(operation, sources, params, verifier=None):
    ...
    if operation == 'spectral_intersection':
        if params or type(sources) is not list or not sources:
            raise ValueError('Intersection requires a nonempty list and no parameters')
        claims = [_source_claim(s, verifier) for s in sources]
        first = _same_target(claims, 'spectrum')
        return _spectral_claim(first['function'], first['mean_zero'],
                               max(rational(c['lower']) for c in claims),
                               min(rational(c['upper']) for c in claims))
    if operation == 'approximation_select':
        if params or type(sources) is not list or not sources:
            raise ValueError('Selection requires a nonempty list and no parameters')
        claims = [_source_claim(s, verifier) for s in sources]
        first = _same_target(claims, 'approximation')
        index = min(range(len(claims)), key=lambda i: rational(claims[i]['error_upper']))
        return {**_approximation_claim(first['function'], claims[index]['error_upper']),
                'selected_source_index': index}
    if operation in ('spectral_affine', 'approximation_affine'):
        if set(params) != {'axis', 'factor', 'shift'}:
            raise ValueError('Affine transport requires axis, factor, shift')
        old = _one_source(sources, verifier)
        expected = 'spectrum' if operation == 'spectral_affine' else 'approximation'
        if old['kind'] != expected:
            raise ValueError('Affine proof has the wrong source kind')
        factor, shift = rational(params['factor']), rational(params['shift'])
        q = dict(old['function'], axis=params['axis'],
                 amplitude=str(factor*rational(old['function']['amplitude'])),
                 offset=str(factor*rational(old['function']['offset'])+shift))
        if expected == 'spectrum':
            if factor != 1:
                raise ValueError('Potential scaling is not spectral scaling')
            return _spectral_claim(q, old['mean_zero'], rational(old['lower'])+shift,
                                   rational(old['upper'])+shift)
        return _approximation_claim(q, abs(factor)*rational(old['error_upper']))
...
def _make(operation, sources, params, verifier=None):
    if type(operation) is not str or type(sources) is not list or type(params) is not dict:
        raise ValueError('Malformed proof transport request')
    if len(sources) > MAX_SOURCES or any(type(s) is not dict for s in sources):
        raise ValueError('Source certificate budget exceeded or malformed')
...
def _guard(certificate):
    ...
    while stack:
        ...
        if node.get('format') != FORMAT:
            continue
        nodes += 1
        if depth > MAX_TRANSPORT_DEPTH or nodes > MAX_TRANSPORT_NODES:
            raise ValueError('Transport proof tree exceeds replay budget')
        sources = node.get('sources')
        if type(sources) is not list or len(sources) > MAX_SOURCES:
            raise ValueError('Malformed or oversized source list')
        if any(type(source) is not dict for source in sources):
            raise ValueError('Source certificates must be objects')
        stack.extend((source, depth+1) for source in sources)


def verify(certificate, expected_function=None, expected_mean_zero=None,
           expected_tolerance=None, verifier=None):
    """Replay mathematics and optionally check an external target requirement.

    expected_tolerance checks the resulting interval width or L2 bound; it does
    not claim that any embedded source was generated for that target.
    verifier is a trusted mathematical verifier for additional source formats.
    """
    try:
        if type(certificate) is not dict or certificate.get('format') != FORMAT:
            return False
        encoded = backend.canonical(certificate)
        _guard(certificate)
        if expected_function is not None and function(expected_function) != certificate['function']:
            return False
        if expected_mean_zero is not None:
            if (certificate.get('kind') != 'spectrum' or type(expected_mean_zero) is not bool
                    or certificate.get('mean_zero') is not expected_mean_zero):
                return False
        if expected_tolerance is not None:
            tolerance = rational(expected_tolerance)
            if tolerance <= 0:
                return False
            error = rational(certificate['exact_width'] if certificate.get('kind') == 'spectrum'
                             else certificate['error_upper'])
            if error > tolerance:
                return False
        rebuilt = _make(certificate['operation'], certificate['sources'],
                        certificate['parameters'], verifier)
        return encoded == backend.canonical(rebuilt)
    except (ValueError, TypeError, KeyError, ArithmeticError, IndexError, RecursionError):
        return False


def constant_for_function(source_function, mean_zero=True):
    """Preserve the caller's profile/axis instead of silently changing target."""
    q = function(source_function)
    return _make('constant_spectrum', [], {'function': q, 'mean_zero': mean_zero})
...
def evaluate(certificate, t, verifier=None):
    ...
    source = certificate['sources'][certificate.get('selected_source_index', 0)]
    if source.get('format') == FORMAT:
    ...
    if operation == 'approximation_affine':
        value = rational(certificate['parameters']['factor'])*value + rational(certificate['parameters']['shift'])
    return value
...
def spectral_intersection(sources, verifier=None):
    return _make('spectral_intersection', sources, {}, verifier)


def approximation_select(sources, verifier=None):
    return _make('approximation_select', sources, {}, verifier)


def normalize_transport(certificate, verifier=None):
    """Replay a chain, then compose its affine prefix into one checked step."""
    if not verify(certificate, verifier=verifier):
        raise ValueError('Only valid transport chains can be normalized')
    factor, shift, axis = F(1), F(0), None
    root = certificate
    prefix = 'spectral' if certificate['kind'] == 'spectrum' else 'approximation'
    operations = {prefix+'_axis', prefix+'_shift', prefix+'_affine'}
    if prefix == 'approximation':
        operations.add('approximation_scale')
    steps = 0
    while root.get('format') == FORMAT and root['operation'] in operations:
        operation, params = root['operation'], root['parameters']
        if 'axis' in params and axis is None:
            axis = params['axis']
        if operation.endswith('_shift'):
            shift += factor*rational(params['shift'])
        elif operation.endswith('_scale'):
            factor *= rational(params['factor'])
        elif operation.endswith('_affine'):
            shift += factor*rational(params['shift'])
            factor *= rational(params['factor'])
        root = root['sources'][0]
        steps += 1
    ...
    if axis is None:
        axis = root['function']['axis']
    normalized = _make(prefix+'_affine', [root],
                       {'axis': axis, 'factor': str(factor), 'shift': str(shift)}, verifier)
    if normalized['function'] != certificate['function']:
        raise ArithmeticError('Composed transform changed the target')
    return normalized
