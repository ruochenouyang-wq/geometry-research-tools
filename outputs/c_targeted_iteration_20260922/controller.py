"""C3: candidate assembly reuse, optional fast candidates, active sectors.

Every native result is serialized and replayed by the unmodified task-bound
verifier. An assembled candidate is never acceptance evidence on its own.
"""
from fractions import Fraction as F
import json
from time import perf_counter
from runtime import baseline, c1, Deadline, Execution, verify, step_component

VERSION = 'c3-targeted-proof-cost-and-active-sector-v1'
...
def solve(raw_task, progress=None):
    ...
    route, native_kind = 'native_c3', None
    ...
    try:
        ...
        if task['kind'] == 'approximation':
            ...
            if not -F(1,2) < alpha < 0:
                ...
                native_kind = 'hybrid_fullspace_deferred_assembly'
                extension = execution.run('strong_fullspace_search', lambda:
                    strong_search.solve(task, wall_seconds=execution.remaining()*0.7,
            ...
            elif task['mean_zero'] and amplitude > 0 and centered_available(q):
                import mean_zero
                native_kind = 'active_mean_zero_sector'
                execution.reserve_fallback()
                try:
                    certificate = execution.run('active_sector_search', lambda:
                        mean_zero.search(task, execution))
                    skip_fallback = certificate is not None
                    if certificate is None:
                        fallback_reason = 'active_sector_search_no_certificate'
                except (ValueError, ArithmeticError, Deadline) as error:
                    fallback_reason = ('active_sector_stage_budget' if isinstance(error, Deadline)
                                       else 'active_sector_precondition_or_candidate_failure')
                    failure_stage = 'active_sector_search'
                    execution.note(fallback_reason, error=type(error).__name__, reason=str(error))
                finally:
                    execution.release_reserve()
            elif not task['mean_zero']:
                import weak_search
                native_kind = 'weak_fullspace_deferred_assembly'
                execution.reserve_fallback()
                try:
                    certificate = execution.run('weak_fullspace_search', lambda:
                        weak_search.search(task, execution))
                    if certificate is None:
                        fallback_reason = 'weak_fullspace_schedule_exhausted'
                except (ValueError, ArithmeticError, Deadline) as error:
                    fallback_reason = ('adaptive_stage_budget' if isinstance(error, Deadline)
                                       else 'weak_search_precondition_or_candidate_failure')
                    failure_stage = 'weak_fullspace_search'
                    execution.note(fallback_reason, error=type(error).__name__, reason=str(error))
                finally:
                    execution.release_reserve()
            else:
